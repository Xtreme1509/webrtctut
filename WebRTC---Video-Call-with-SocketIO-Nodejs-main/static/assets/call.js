...
function processAccept() {
    if (!remoteRTCMessage) {
        // If the remoteRTCMessage is not available, it means the other user is offline.
        c();
        return;
    }
    peerConnection.setRemoteDescription(new RTCSessionDescription(remoteRTCMessage));
...
function handleIceCandidate(event) {
    ...
    if (event.candidate) {
        ...
        console.log(myName,"is connected to",otherUser,"at time");
...
function stopp() {
    ...
    socket.emit("endCall", { caller: myName, callee: otherUser });
}
...
function d() {
    if (callInProgress) {
        stopp(); // Call the function to stop the ongoing call and clean up resources.
        socket.disconnect();
        handleRemoteStreamRemoved();
